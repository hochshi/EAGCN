...
def train(tasks, EAGCN_structure, n_den1, n_den2, file_name):
    x_all, y_all, target, sizes = load_data(dataset, file_name=dataset_file, dataset_file_format=dataset_file_format)
    max_size = max(sizes)
    x_all, y_all = data_filter(x_all, y_all, target, sizes, tasks)
    BCE_weight = FloatTensor(set_weight(y_all))
    ...
    model_params = {'n_bfeat': n_bfeat, 'n_afeat': 25, 'n_sgc1_1': n_sgc1_1, 'n_sgc1_2': n_sgc1_2, 'n_sgc1_3': n_sgc1_3,
            ...
            'n_sgc2_5': n_sgc2_5,
            'n_den1': n_den1, 'n_den2': n_den2,
            'nclass': model.nclass, 'dropout': dropout}
    with open('{}_model_arguments'.format(file_name), 'wb') as fp:
        pickle.dump(model_params, fp)


parser = argparse.ArgumentParser(description='Run train on dataset and file')
parser.add_argument('--dataset', help='Which dataset should we train on')
parser.add_argument('--file', help='location of dataset file')
parser.add_argument('--long', help='Is the file in wide or long format', type=bool)

args = parser.parse_args()
dataset = args.dataset
dataset_file = args.file
dataset_file_format = args.long
